...
Keep the implementation performant: prefer canvas or a small number of SVG layers, avoid thousands of particles, and do not use physics simulation. Use no external libraries. Use the most efficient complete approach, then use Playwright screenshots to see the output yourself, check for errors, review and adjust the output until it looks correct.
